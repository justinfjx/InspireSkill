...
| 方法 | 路径 | 用途 | CLI 引用 |
| --- | --- | --- | --- |
| `POST` | `{prefix}/project/list` | 列项目 + 剩余预算 + 优先级（实测接受带 `filter` 的 body，**不是完全空 body**；直接传 `{workspace_id:...}` 会被 proto 拒） | `browser_api.projects.list_projects` |
...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| `GET` / `POST` / `WS` | `{prefix}/notebook/lab/{notebook_id}/proxy/{port}/...` | 经由平台代理透传到 notebook 内部 Jupyter 服务的任意 HTTP / WebSocket 请求。`inspire notebook ssh connect` 的 bootstrap（Jupyter terminal REST + WebSocket，失败兜底 Playwright 终端自动化）全走这条 |

...
当前封装覆盖 `inspire model list/status/versions/register`。追加版本、删除、发布到模型广场等仍以平台页面为准。当前前端入口是 `/jobs/modelService?spaceId=<workspace_id>`。
...
当前 `serving` CLI 以 `/jobs/modelDeployment?spaceId=<workspace_id>` 的 Browser API 为准，覆盖 `create/list/status/stop/delete/configs/metrics`。旧 OpenAPI 仍保留在 `platform.openapi.inference_servings` 作为历史 helper，但它的 `image_type + spec_id + image URL` payload 与当前网页创建表单不同，不能作为 CLI create 的事实来源。

网页端部署表单已实测确认：当前入口展示“自定义部署”和“LLM 大语言模型部署”；自定义部署使用注册模型、镜像 `mirror_id`、运行命令、端口、优先级、资源规格、副本数、单副本实例数和共享内存。`resources specs --usage serving` 必须查 `SCHEDULE_CONFIG_TYPE_SERVE`，不是训练任务的 `SCHEDULE_CONFIG_TYPE_TRAIN`。在 `H200-2号机房` 实测到的 serving 规格是 `1,18,200`、`2,36,400`、`4,72,900`、`8,168,1800`。

...
- 能在 OpenAPI 上做且与当前页面合同一致的才走 OpenAPI（`job` / `hpc` 的 create / detail / stop）。`inference_servings` 虽有旧 OpenAPI helper，但当前 `serving` CLI 按网页真实请求走 Browser API：`mirror_id`、`resource_spec_price`、RESTful detail、v2 stop/start Action。
...
本文档不是“看到过的所有 URL 堆表”。每个 Browser API 条目必须能被归入一个明确状态，避免把猜测当成实现合同。

| 状态 | 含义 | 可以据此做什么 |
| --- | --- | --- |
| **CLI 闭环** | 已有 wrapper / CLI 入口，body 和主要响应字段写清楚，并有单元测试、live smoke 或真实使用记录支撑 | 可以作为 CLI 行为合同维护 |
| **前端实测** | 从当前页面抓包或 bundle 读到，路径和 body 有证据，但 CLI 还未封装 | 可以作为新增 wrapper 的起点；实现前仍要做最小 live 验证 |
| **候选 / 未解** | 只知道路径存在，或 body / enum / destructive 语义不完整 | 只能作为调研线索，不能直接据此写命令 |
| **退役** | 平台已 404 或前端不再调用，保留是为了防回归和解释历史迁移 | 不能新增依赖；发现复活时重新验证 |

读表规则：

- `CLI 引用` 列有 `browser_api.*` 或 `inspire ...` 的，才算 CLI 侧闭环。
- `CLI 引用` 为 `——`、`暂未封装` 或正文标明“未解”的，只代表前端观察或候选信息，不代表可直接调用。
- destructive 端点（create / delete / stop / save / publish）不能靠 read-only reverse capture 证明完整合同；必须有受控 live smoke、bundle payload 或现有 CLI 测试补足。
- 新增或修改 Browser API wrapper 时，同时更新本文档、[`cli/scripts/reverse_capture/known_endpoints.py`](../../cli/scripts/reverse_capture/known_endpoints.py) 和相关测试。三者不一致时，以当前前端真实请求为准。

当前已知不闭合面：

- 模型仓库只闭合 `list/status/versions/register`。追加版本、删除、编辑元数据、发布模型广场仍是页面功能，不能从本文档推导 CLI 合同。
- 模型部署只闭合“自定义部署”的 create / list / status / stop / delete。LLM 一键部署、模型广场来源部署、更新部署配置仍需单独实测。
- `POST {prefix}/compute_groups/list` 只确认路径存在，body schema 未闭合；资源查询不要使用它，继续用 `logic_compute_groups/list` 和 `resource_prices/logic_compute_groups/`。
- SSH key 管理只记录到入口路径，`ssh/create` body 未解，CLI 不封装。
- Ray 聚合日志路径已观察到，但 CLI 暂未封装；当前诊断面仍是 `ray events` / `ray status` / 实例视图。

## 端点清单

下面按域列出 Browser API 端点。表格同时包含 CLI 闭环端点、前端实测但未封装端点，以及少量候选 / 退役说明。`{prefix}` 默认 `/api/v1`。
...
| `GET` | `{prefix}/notebook/schedule/{workspace_id}` | notebook schedule 配置；UI 使用 path-param 形态，CLI helper 先试这条 | `browser_api.notebooks.get_notebook_schedule`；`inspire init --discover` |
...
| `POST` | `{prefix}/compute_groups/list` | 候选旧资源接口；实测对 `{workspace_id}` body 返回 400，body schema 未闭合 | 候选 / 未解，不用于 CLI |
...
当前封装覆盖 `inspire model list/status/versions/register`。追加版本、删除、编辑元数据、发布到模型广场等仍以平台页面为准，不属于当前 CLI 合同。当前前端入口是 `/jobs/modelService?spaceId=<workspace_id>`。

网页端注册表单已实测确认：模型名支持字母、数字、下划线、短横线和点，且必须以字母开头；模型存储位置必须是当前空间下的项目目录路径，否则后端会返回“模型源路径不存在或访问异常”。

日常默认看人类输出；结构化输出只用于脚本。平台的 `list` 端点会把最新版本属性合入模型条目，而 `detail` 端点只返回模型主记录，所以 `is_vllm_compatible` 在 `list` 与 `status` 中可能不一致。要准确的版本属性，用 `inspire model versions <model-name>`。

| 方法 | 路径 | 用途 | CLI 引用 |
| --- | --- | --- | --- |
| `POST` | `{prefix}/model/list` | 工作空间下的模型注册表。body: `{page, page_size, filter_by:{keyword?, user_id?, project_id?[]}, workspace_id}`。`user_id` 必须是字符串；`project_id` 必须是数组；`keyword` 可做服务端名称搜索 | `browser_api.models.list_models`；`inspire model list` |
...
网页端部署表单已实测确认：当前入口展示“自定义部署”和“LLM 大语言模型部署”；自定义部署使用注册模型、镜像 `mirror_id`、运行命令、端口、优先级、资源规格、副本数、单副本实例数和共享内存。`resources specs --usage serving` 必须查 `SCHEDULE_CONFIG_TYPE_SERVE`，不是训练任务的 `SCHEDULE_CONFIG_TYPE_TRAIN`。在 `H200-2号机房` 实测到的 serving 规格是 `1,18,200`、`2,36,400`、`4,72,900`、`8,168,1800`。LLM 一键部署与模型广场来源部署尚未闭合，不能套用自定义部署 payload。

| 方法 | 路径 | 用途 | CLI 引用 |
| --- | --- | --- | --- |
| `POST` | `{prefix}/inference_servings/list` | 列部署。body: `{page, page_size, filter_by:{my_serving, keyword?, project_id?[], status?[]}, workspace_id}`；返回 `{inference_servings[], total}` | `browser_api.servings.list_servings`；`inspire serving list` |
...
| `GET` | `{prefix}/inference_servings/{id}` | 部署详情；返回中 `model`、`mirror`、`resource_spec_price` 常是嵌套对象，human formatter 必须取名称而不是直接打印 dict | `browser_api.servings.get_serving_detail`；`inspire serving status` |
...
**CLI 目前不碰平台层 SSH key**（`inspire` 的 SSH 用户功能是在 notebook 内部起 dropbear，相关实现见 [`rtunnel.py`](../../cli/inspire/platform/web/browser_api/rtunnel.py)），但平台自带一套 SSH key 管理：
...
| `POST` | `{prefix}/ssh/create` | 添加 SSH key。body 字段名未解出 | 候选 / 未解，不用于 CLI |
...
- **认证依赖 Playwright**。如果你的环境装不了 Chromium（headless 容器、严格沙盒），Browser API 这一整层就不能用；OpenAPI 那 10 条业务端点还能走。
...
- **闭环优先于覆盖面**。宁可把未知字段标成“候选 / 未解”，也不要为了表格完整写一个不可执行的 body。实现 CLI 前至少要有 wrapper 测试；涉及创建 / 删除 / 停止时还要有受控 live smoke 或 bundle payload 证据。

...
启智平台网页 `qz.sii.edu.cn` 是你日常实验链路里最慢的那一环——每次申请资源、新建 notebook、开 SSH、同步代码都要反复点点点。
...
启智官方的 **InspireCode** 是把 OpenCode 直接部署到某个 Inspire 实例里——要用就得打开 `qz.sii.edu.cn`、进那个实例、在它的终端里跟 OpenCode 对话。凡是"把 Agent 装在服务器上"的方案都是这个路数。**InspireSkill 走相反路径：Agent 留在你本机，Inspire 降格为被调用的工具。**
...
| **入口** | 必须打开 `qz.sii.edu.cn` 的实例终端 | 你本来就在用的 Claude Code / Codex / Gemini CLI / OpenClaw / OpenCode |
...
| **自动化 / 可复现** | 对话历史锁在浏览器页面里 | 命令流可保存 / 回放；人类格式给 Agent 决策，结构化输出留给脚本消费 |
...
平台侧行为突变又未及时 patch 时，在 [issue tracker](https://github.com/realZillionX/InspireSkill/issues) 开一条，附 `inspire --debug <cmd>` 的 trace（CLI 会自动脱敏敏感登录凭据和代理信息）。**反馈流程的更多细节见下方"开发与贡献"一节。**
